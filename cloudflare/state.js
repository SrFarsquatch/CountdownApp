...
const normalizeParticipantIds=value=>Array.isArray(value)?[...new Set(value.map(String).filter(Boolean))].slice(0,50):[];
export function normalizeQuestEvent(x={}){
  const created=iso(x.created)||new Date().toISOString(),start=iso(x.start)||new Date().toISOString(),allDay=Boolean(x.allDay);
  const fallbackEnd=new Date(new Date(start).getTime()+(allDay?86400000:3600000)).toISOString();
  return{id:String(x.id||id()),title:cleanText(x.title||'Event',200),description:cleanText(x.description,4000),location:cleanText(x.location,500),start,end:iso(x.end)||fallbackEnd,allDay,calendarId:x.calendarId?String(x.calendarId):'',participantIds:normalizeParticipantIds(x.participantIds),created,updated:iso(x.updated)||created};
}
export function normalizeCountdown(x={}){
  const created=iso(x.created)||new Date().toISOString();
  return{id:String(x.id||id()),name:cleanText(x.name||'Countdown',100),end:iso(x.end)||new Date(Date.now()+86400000).toISOString(),created,accentColor:en(x.accentColor,COLORS,'black'),progressMode:en(x.progressMode,PROGRESS_MODES,'time'),progressStyle:en(x.progressStyle,PROGRESS_STYLES,'solid'),progressStart:iso(x.progressStart)||created,progressCurrent:num(x.progressCurrent,0),progressTotal:Math.max(0,num(x.progressTotal,100)),dateDisplayStyle:en(x.dateDisplayStyle,DATE_STYLES,'medium'),timeDisplayStyle:en(x.timeDisplayStyle,TIME_STYLES,'days'),showExactDate:x.showExactDate!==false,showProgressBar:x.showProgressBar!==false,displayEnabled:x.displayEnabled!==false,pinned:Boolean(x.pinned),goalId:x.goalId?String(x.goalId):'',calendarId:x.calendarId?String(x.calendarId):'',participantIds:normalizeParticipantIds(x.participantIds)};
}
